import { useThemeStore } from '../../store/theme.store'

interface MessageContextMenuProps {
  x: number
  y: number
  isOwn: boolean
  messageType: string
  fileUrl?: string
  content: string
  onClose: () => void
  onCopy: () => void
  onEdit?: () => void
  onDelete?: () => void
  onSave?: () => void
}

export default function MessageContextMenu({
  x,
  y,
  isOwn,
  messageType,
  fileUrl,
  content,
  onClose,
  onCopy,
  onEdit,
  onDelete,
  onSave
}: MessageContextMenuProps) {
  const { theme } = useThemeStore()
  const isDark = theme === 'dark'

  const handleCopy = () => {
    if (messageType === 'text' || messageType === 'sticker') {
      navigator.clipboard.writeText(content)
    }
    onCopy()
    onClose()
  }

  const handleSave = () => {
    if (fileUrl) {
      const link = document.createElement('a')
      link.href = `http://localhost:3001${fileUrl}`
      link.download = content || 'download'
      link.target = '_blank'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    }
    onSave?.()
    onClose()
  }

  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 z-40" onClick={onClose} />
      
      {/* Menu */}
      <div 
        className={`fixed z-50 min-w-[160px] rounded-xl shadow-xl overflow-hidden ${
          isDark ? 'bg-[#17212b] border border-[#232e3c]' : 'bg-white border border-[#dddfe2]'
        }`}
        style={{ 
          left: Math.min(x, window.innerWidth - 180), 
          top: Math.min(y, window.innerHeight - 250) 
        }}
      >
        {/* Copy */}
        {(messageType === 'text' || messageType === 'sticker') && (
          <button
            onClick={handleCopy}
            className={`w-full px-4 py-2.5 flex items-center gap-3 transition-colors ${
              isDark ? 'text-white hover:bg-[#232e3c]' : 'text-black hover:bg-[#f0f2f5]'
            }`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
            </svg>
            <span className="text-sm">Copy</span>
          </button>
        )}

        {/* Save (for media) */}
        {(messageType === 'image' || messageType === 'video' || messageType === 'file') && fileUrl && (
          <button
            onClick={handleSave}
            className={`w-full px-4 py-2.5 flex items-center gap-3 transition-colors ${
              isDark ? 'text-white hover:bg-[#232e3c]' : 'text-black hover:bg-[#f0f2f5]'
            }`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
            </svg>
            <span className="text-sm">Save</span>
          </button>
        )}

        {/* Edit (only for own text messages) */}
        {isOwn && messageType === 'text' && onEdit && (
          <button
            onClick={() => { onEdit(); onClose(); }}
            className={`w-full px-4 py-2.5 flex items-center gap-3 transition-colors ${
              isDark ? 'text-white hover:bg-[#232e3c]' : 'text-black hover:bg-[#f0f2f5]'
            }`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            <span className="text-sm">Edit</span>
          </button>
        )}

        {/* Delete (only for own messages) */}
        {isOwn && onDelete && (
          <button
            onClick={() => { onDelete(); onClose(); }}
            className={`w-full px-4 py-2.5 flex items-center gap-3 text-[#e53935] transition-colors ${
              isDark ? 'hover:bg-[#e53935]/10' : 'hover:bg-[#e53935]/10'
            }`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
            <span className="text-sm">Delete</span>
          </button>
        )}
      </div>
    </>
  )
}
